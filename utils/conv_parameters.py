...
def get_conv_parameters(model):
    """
    Function copied from print_cfg in pytorch_YOLOv4.tool.config
    :param model: DarkNet model
    :return: only widths and heights of conv layers
    """
    blocks = model.blocks
    ...
    conv_filters = [prev_filters]
    conv_widths = [prev_width]
    conv_heights = [prev_height]
    strides = [None]
    kernel_sizes = [None]
    ...
    for block in blocks:
        ...
        if block['type'] == 'net':
            ...
            conv_filters.append(prev_filters)
            conv_widths.append(prev_width)
            conv_heights.append(prev_height)
    ...
    return conv_filters, conv_widths, conv_heights, strides, kernel_sizes
